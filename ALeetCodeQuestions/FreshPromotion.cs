...
namespace ALQ
{
    ...
    {
        public int FreshPromotionMethod(string[][] codeList, string[] shoppingCart)
        {
            int inputLength = shoppingCart.Length;
            int codeListIndex = 0;
            int result = 0;
            for(int i=0;i<inputLength;i++)
            {
                //if a match is found start an inner loop to see if this can be a potential group
                if(codeList[codeListIndex][0]==shoppingCart[i] || codeList[codeListIndex][0]=="anything")
                {
                    int tempI = i + 1;
                    bool flag = true;
                    for (int j=1;j<codeList[codeListIndex].Length;j++,tempI++)
                    {
                        //if a mismatch is found break
                        if(codeList[codeListIndex][j]!=shoppingCart[tempI] && codeList[codeListIndex][j]!="anything")
                        {
                            flag = false;
                            break;
                        }
                    }
                    //if flag is true it means the matching was complete and now we can continue looking for the next group
                    if(flag)
                    {
                        i = tempI-1;
                        codeListIndex++;
            ...
            if(codeListIndex==codeList.Length)
            {
                result = 1;
            }
            return result;
